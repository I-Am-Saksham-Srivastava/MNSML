...
void main() {
  ...
  test('TodoList JSON serialization/(de)serialization', () {
    ...
    const targetJson = <String, dynamic>{
      'todos': [
        {'description': 'first one', 'done': false},
        {'description': 'second one', 'done': false},
      ],
      'filter': 'all',
    };
  ...
  group('when filter is set to "all"', () {
    ...
    test(
      'when there are no completed todos, can-remove-all-completed is false',
      () {
    ...
    test(
      'when there are no pending todos, can-mark-all-completed is false',
      () {
