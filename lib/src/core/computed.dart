...
class Computed<T> extends Atom implements Derivation, ObservableValue<T> {
  ...
  /// mnsml uses a 2-phase change propagation that ensures no unnecessary computations are performed.
  factory Computed(
    T Function() fn, {
    String? name,
  ...
  @override
  mnsmlCaughtException? _errorValue;

  @override
  mnsmlCaughtException? get errorValue => _errorValue;
  ...
  T get value {
    if (_isComputing) {
      throw mnsmlCyclicReactionException(
    ...
    if (track) {
      ...
      if (_context.config.disableErrorBoundaries == true) {
        ...
        try {
          ...
          _errorValue = mnsmlCaughtException(e, stackTrace: s);
        }
      }
    }
