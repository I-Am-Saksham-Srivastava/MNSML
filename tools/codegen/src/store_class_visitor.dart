...
import 'package:mnsml/src/api/annotations.dart'
...
import '../src/template/method_override.dart';
import '../src/template/observable.dart';
import '../src/template/observable_future.dart';
import '../src/template/observable_stream.dart';
import '../src/template/store.dart';
import '../src/template/util.dart';
import '../src/type_names.dart';
import '../src/utils/non_private_name_extension.dart';
...
class StoreClassVisitor extends SimpleElementVisitor2 {
  StoreClassVisitor(
    ...
    _storeTemplate =
        template
          ..typeParams.templates.addAll(
            userClass.typeParameters.map(
              (type) => typeParamTemplate(type, typeNameFinder),
            ),
          )
  ...
  final _observableChecker = const TypeChecker.typeNamed(
    MakeObservable,
    inPackage: 'MNSML',
  );

  final _computedChecker = const TypeChecker.typeNamed(
    ComputedMethod,
    inPackage: 'MNSML',
  );

  final _actionChecker = const TypeChecker.typeNamed(
    MakeAction,
    inPackage: 'MNSML',
  );
  ...
  void visitClassElement(ClassElement element) {
    if (isMixinStoreClass(element)) {
      errors.nonAbstractStoreMixinDeclarations.addIf(
        !element.isAbstract,
        element.name!,
      );
    }
  ...
  void visitPropertyAccessorElement(PropertyAccessorElement element) {
    ...
    if (element is! GetterElement ||
        !_computedChecker.hasAnnotationOfExact(element)) {
      return;
    }
  ...
  void visitMethodElement(MethodElement element) {
    ...
    if (_actionChecker.hasAnnotationOfExact(element)) {
      ...
      if (element.fragments.any((fragment) => fragment.isAsynchronous)) {
...
const _storeMixinChecker = TypeChecker.typeNamed(Store, inPackage: 'MNSML');
const _toStringAnnotationChecker = TypeChecker.typeNamed(
  StoreConfig,
  inPackage: 'MNSML',
);
...
bool hasGeneratedToString(BuilderOptions options, ClassElement? classElement) {
  ...
  if (classElement != null && isStoreConfigAnnotatedStoreClass(classElement)) {
    final annotation = _toStringAnnotationChecker.firstAnnotationOfExact(
      classElement,
    );
